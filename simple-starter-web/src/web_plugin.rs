...
use simple_starter_core::tracing::Level;
use simple_starter_core::{AppContext, Extensions, Plugin};
use toml::Value;
...
/// # 三阶段生命周期
///
/// - `assemble`: 创建 `WebExtensionRegistry` 并放入扩展存储，
...
impl Plugin for WebPlugin {
    ...
    /// 装配期
    ///
    /// 将 `WebPlugin` 自身的 `WebExtensionRegistry` 移入扩展存储，
    ...
    /// （包含用户在 `new()` 阶段注册的扩展以及其他插件通过 `AppContext` 上下文注册的扩展）。
    /// 从组件容器获取监听器工厂、消费注册表、注册后台任务。
    async fn finalize(&mut self, ctx: &mut AppContext) -> simple_starter_core::anyhow::Result<()> {
        // 1. 加载 Web 配置（经 AppContext）
        let container = ctx.container().clone();
        let web_config: WebConfig = ctx
            .get_config_to_struct::<WebConfig>("web")
            .context("Failed to load 'web' config section")?;

        // 2. 快照后台任务与请求期无法访问上下文的配置：
        //    - 日志等级（HTTP TraceLayer 使用）
        //    - 服务名（json_response 请求期读取）
        let log_level: Level = ctx
            .get_config_value_by_path("logger.level")
            .context("Failed to load 'logger.level' config")?
            .as_str()
            .context("Failed to parse log level as string")?
            .parse()
            .context("Invalid log level format")?;
        let service_name: Option<String> = ctx
            .get_config_value_by_path("app.name")
            .and_then(|v| v.as_str())
            .map(|s| s.to_string());
        crate::support::json_response::set_service_name(service_name);
        ...
        // 5. 从组件容器获取监听器工厂（默认实现经条件注册保证存在，用户实现存在时自动退位）
        let listener_factory = container
            .get_component_by_trait::<dyn TcpListenerFactory>()
            .context("TcpListenerFactory component not found in component container")?;
